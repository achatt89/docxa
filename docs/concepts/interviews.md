...
Docxa includes several pre-defined roles, each with its own focus area:
...
1.  **Select a Template**: Docxa looks at the document template (e.g., `prd.template.json`) to see what information is missing.
2.  **Dynamic Interview Generation**: Docxa selects an interview definition (e.g., `prd.product_manager.interview.json`) that matches the role.
...
Once an interview is completed, its answers are normalized and used as evidence. If you change a requirement in the code, Docxa can identify if a new interview is needed to update the documentation.
