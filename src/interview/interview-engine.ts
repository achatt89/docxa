...
export class InterviewEngine {
    ...
    async startInterview(documentId: string, roleId: string, stakeholderName?: string): Promise<InterviewSession> {
        const definition = await this.loader.getByDocumentAndRole(this.interviewDir, documentId, roleId);
        if (!definition) {
            throw new Error(`No interview definition found for document ${documentId} and role ${roleId}`);
        }
    ...
    async getNextQuestion(session: InterviewSession): Promise<InterviewQuestion | undefined> {
        const definition = await this.loader.getByDocumentAndRole(this.interviewDir, session.documentId, session.roleId);
        if (!definition) return undefined;

        return this.strategy.getNextQuestion(definition, session);
    }

    async registerAnswer(session: InterviewSession, questionId: string, rawAnswer: any): Promise<InterviewSession> {
        const definition = await this.loader.getByDocumentAndRole(this.interviewDir, session.documentId, session.roleId);
