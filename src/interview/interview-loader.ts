...
export class InterviewLoader {
          ...
          try {
            ...
            if (this.templateSystem) {
              ...
              if (template) {
                ...
                console.warn(
                  `Skipping alignment validation for ${validated.documentId}: Template not found.`,
                );
              }
            }
            ...
            throw new Error(
              `Interview definition validation failed for ${fullPath}: ${error.message}`,
              { cause: error },
            );
          }
        }
  ...
  async getByDocumentAndRole(
    ...
    return all.find(
      (i) => i.documentId.toUpperCase() === documentId.toUpperCase() && i.roleId === roleId,
    );
  }
}
