...
export type EvidenceSource = 'document' | 'interview' | 'repository_analysis' | 'manual_context';
    ...
    for (const req of requirements) {
      ...
      for (const option of req.satisfiesWith) {
        if (option.sourceType === 'document' && existingDocs.includes(option.sourceId)) {
          ...
          option.sourceType === 'interview' &&
          sessions.some((s) => s.roleId === option.sourceId && s.status === 'completed')
        ) {
          satisfied = true;
          alternativeEvidenceUsed.push(`${req.key} (Interview: ${option.sourceId})`);
        } else if (
          option.sourceType === 'repository_analysis' &&
          analysisAvailable.includes(option.sourceId)
        ) {
          satisfied = true;
  ...
  private addSuggestions(
    ...
    for (const option of requirement.satisfiesWith) {
      if (option.sourceType === 'interview') {
        suggestions.push(
          `Start ${documentId} interview with role ${option.sourceId} to satisfy ${requirement.key}`,
        );
      } else if (option.sourceType === 'repository_analysis') {
        suggestions.push(
          `Run 'docxa discover' to collect repository analysis for ${requirement.key} (${option.sourceId})`,
        );
